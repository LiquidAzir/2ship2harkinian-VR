// OpenXR VR support for 2 Ship 2 Harkinian (Majora's Mask PC port on libultraship / Fast3D).
//
// Ported from RaYRoD-TV/BanjoKazooie-VR (src/port/vr/vr.h), kept API-identical so the shared
// eye-matrix / menu / interpreter glue moves between the ports unchanged. Public interface is
...
// VR view modes (CVar gVRViewMode). The eye-matrix builder and the render path branch on this.
// Slot 2 (the donor layout's Cockpit) has no MM equivalent and is skipped by the in-game cycle;
...
typedef enum {
    ...
    VR_VIEW_FIRST_PERSON = 1, // eye pushed forward to Link's head, life-size stereo (Link hidden)
...
// this for the game's perspective projection load; MM's camera lives on the modelview stack, so the
...
// This projection LOAD is the sky pass's: the game side registers the Mtx address of the sky's
// perspective (View_ApplyPerspective's skybox pass) each frame, and the interpreter asks per
// perspective load. Defined game-side (VrGame.cpp) - the matching policy is the game's.
...
// True when the 3D sky dome replaces the flat 2D background: the CVar gVRSkyDome is on and a dome DL
// has been built. Drives the interpreter's dome pass and tells the game to skip its own flat backdrop.
...
// The game pushes the player camera each frame (world-space eye/at/up, game units) so the dome's
// view-proj carries the game camera's orientation as well as the head's - it sweeps as the camera
// turns and holds still as you look around, anchored on yaw and pitch.
...
// units. The tabletop is anchored on the FOCUS (Link) rather than the camera, so he stays put on
...
// FLATSCREEN-ON-A-PANEL: for non-gameplay screens (title/file select/map/pause), render the flat frame
// once into the panel swapchain and present it on one large head-locked quad.
...
// VR view mode (Third Person / First Person / Theater / Diorama). See VrViewMode. The render path
// routes Theater to the flat panel path, the others to the per-eye stereo path.
...
// In-game 2D HUD (hearts, magic, clock, buttons): repositioned onto a head-locked plane in the
// eye render.
...
// perspective clip-Z through the ordering scale reads as metres of depth and goes cross-eyed. In MM
// this is the C-button item models, the mask icons and the three-day clock.
...
// so the headset rolls with Goron Link's roll, spins with the Deku flower launch, is knocked about
// by a hit, and lies back on death. Fed per game tick from the player state machine (its own
// animation clock wherever there is one to read); 0 is upright, positive pitches forward (nose
// down). Eased to the headset's frame rate inside, and ignored outside first person.
...
// GAME owns it (dialogue, cutscenes, scripted reveals), the First Person framing offsets fade out so
// an authored shot is shown as authored instead of being pushed a metre into the nearest wall.
void vr_set_fp_framing(int on);

void vr_shutdown(void);

// --- headless verification seam ------------------------------------------------
// The per-eye render path must be provable WITHOUT a headset: these build the same matrix set
// vr_begin_frame would (identity head orientation, symmetric fov, current scale knobs, and the
// LIVE per-eye separation math around the centre) so the interpreter's RunVrEye can run headless
// per eye, and dump its texture to a BMP on disk. Driven by the MM_VR_EYEDUMP env var; no effect
// on normal play. eye: 0 = left, 1 = right.
// MM_VR_FOVTEST="l,r,u,d" (degrees, signed) overrides the synthetic fov with an asymmetric one, so
